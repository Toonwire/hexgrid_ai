...
function onResize() {
	// TODO: remake hexagons with current state, but different dimens - how though?
}

function compileEditorCode() {
	// save code in editor to variable
	editorCode = getEditorCode();

	// hide editor & show selection screen
	hideEditor();
	
}

function downloadEditorCode() {
	var blob = new Blob([getEditorCode()], {type: "text/javascript;charset=utf-8"});
	saveAs(blob, "custom_algo.js");	// FileSaver.js
}
